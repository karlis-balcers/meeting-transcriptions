...
func Defaults() Config {
	return Config{
		...
		Audio: AudioConfig{
			...
			FrameDurationMS:      100,
			SilenceThreshold:     50,
			SilenceDuration:      NewDuration(time.Second),
			MaxSegmentDuration:   NewDuration(300 * time.Second),
		},
		Paths: PathsConfig{},
...
func Load(path string, explicitPath bool, env map[string]string) (Config, []string, error) {
	cfg := Defaults()
	...
	if err != nil {
		...
		return cfg, warnings, fmt.Errorf("read config %s: %w", path, err)
	}
	...
	if err := yaml.Unmarshal(content, &cfg); err != nil {
		return cfg, warnings, fmt.Errorf("parse config %s: %w", path, err)
	}
	return cfg, warnings, cfg.Validate()
}
