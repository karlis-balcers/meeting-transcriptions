...
func TestWindowsAudioEndpointRenderCandidatesBecomeLoopbackOutputs(t *testing.T) {
	const (
	...
	formatted := FormatDevices(devices)
	if !containsAll(formatted, "Speakers (Realtek(R) Audio) [Loopback]", "Headphones (Jabra Link 380) [Loopback]", `aliases=`+speakersID, `aliases=`+headphonesID) {
...
func TestWindowsDShowMicOnlyListDoesNotExposeOutputCapture(t *testing.T) {
	...
	if len(warnings) != 1 || !containsAll(warnings[0], "loopback", "output-capture") {
		t.Fatalf("mic-only Windows discovery should warn about missing loopback output capture, got %v", warnings)
	}
	var outputs []Device
	for _, device := range devices {
		if device.Source == SourceOutput {
...
func TestWindowsDirectShowMicOnlyCanSupplementLoopbackEndpointOutput(t *testing.T) {
	...
	if outputs[0].ID != "Speakers (Realtek(R) Audio)" || outputs[0].Name != "Speakers (Realtek(R) Audio) [Loopback]" || len(outputs[0].Aliases) != 1 {
