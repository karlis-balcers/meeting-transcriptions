...
func (d SystemDiscoverer) listLinux(ctx context.Context) ([]Device, []string, error) {
	...
	if infoErr != nil || sourcesErr != nil {
		...
		if !hasSource(devices, SourceMic) {
			devices = append(devices, Device{ID: "default", Name: "PulseAudio default microphone", Source: SourceMic, Default: true, Backend: BackendPulse})
		}
	...
	devices = appendConfiguredIfMissing(devices, d.Preferences, BackendPulse)
	if !hasSource(devices, SourceMic) {
		devices = append(devices, Device{ID: "default", Name: "PulseAudio default microphone", Source: SourceMic, Default: true, Backend: BackendPulse})
...
func (d SystemDiscoverer) listDarwin() []Device {
	devices := []Device{{ID: ":0", Name: "Default AVFoundation microphone", Source: SourceMic, Default: true, Backend: BackendAVFoundation}}
	return appendConfiguredIfMissing(devices, d.Preferences, BackendAVFoundation)
}
...
const windowsOutputCaptureWarning = "Windows did not detect a DirectShow system-output (loopback) capture source for output-capture: enable Stereo Mix in Sound settings, or install a loopback device such as VB-CABLE / Voicemeeter / virtual-audio-capturer, then re-run `transcribe --list-devices`."
...
func appendWindowsConfiguredIfMissing(devices []Device, prefs Preferences) []Device {
	for _, configured := range configuredDevices(prefs, BackendDirectShow) {
...
func ParsePactlSources(sources string, defaults PactlDefaults) []Device {
	...
	for _, rawLine := range strings.Split(sources, "\n") {
		...
		devices = append(devices, Device{ID: id, Name: name, Aliases: aliases, Source: source, Default: defaultDevice, Backend: BackendPulse})
	}
	return devices
}
